...
def load_gcs_to_bigquery(date_str: str) -> None:
    ...
    # 1 Step 1 — delete existing rows for this date
    delete_query = f"""
    DELETE FROM `{table_ref}`
    WHERE DATE(created_at) = '{date_str}'
    """
    client.query(delete_query).result()
    print(f"Deleted existing rows for {date_str}")

    # 2 Step 2 — load new data from GCS
...
if __name__ == "__main__":
    date_str = os.environ.get("BRUIN_START_DATE", "2024-11-01")
